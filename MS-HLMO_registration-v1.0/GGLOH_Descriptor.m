...
% Partial Main Orientation Map (PMOM), 取值范围：[0,pi)
...
for k = 1:size(kps,1)
    ...
    if rot_flag
        ...
        angle_bin(W+y1-y+1:W+y2-y+1, W+x1-x+1:W+x2-x+1) = ...
            mod(floor((PMOM_o(y1:y2, x1:x2)-orient) * NBO/pi), NBO) + 1;  % 取值范围：[1,NBO] & 0
...
%         angle = mod(angle-kps(k,end), pi);  % 取值范围：[0,pi)
        ...
        angle_bin(W+y1-y+1:W+y2-y+1, W+x1-x+1:W+x2-x+1) = ...
            floor(PMOM_o(y1:y2, x1:x2) * NBO/pi) + 1;  % 取值范围：[1,NBO] & 0
    end
