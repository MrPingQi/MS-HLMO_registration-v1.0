...
function descriptors = GGLOH_Descriptor(img, kps, ...
    patchsize, NBS, NBO, int_flag, rot_flag)

W = floor(patchsize/2); % 窗半径
X = -W : W; % 邻域x坐标
Y = -W : W; % 邻域y坐标
[XX,YY] = meshgrid(X,Y);
...
weight = zeros(patchsize+1,patchsize+1);
descriptor = zeros(size(kps,1), (1+2*NBS)*NBO); % descriptor (size: (1+2×NBS)×NBO)
for k = 1:size(kps,1)
    ...
    angle_bin = zeros(patchsize+1,patchsize+1)-1; % -1+1=0 表示不统计
    ...
    for xx = 1:patchsize+1
        for yy = 1:patchsize+1
            Rho_t = Rho(yy,xx);
            Theta_t = Theta(yy,xx);
