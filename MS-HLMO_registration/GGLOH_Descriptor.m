...
%     kps = [kps,diag(PMOM_o(kps(:,2),kps(:,1)))];  % fast
    ...
    Theta = mod(floor(Theta*NA/pi/2),NA)+1; % 取值范围：[1,NBS]
end
...
for k = 1:size(kps,1)
    ...
    if rot_flag
        ...
        Theta = mod(floor(Theta*NA/pi/2),NA)+1; % 取值范围：[1,NBS]
        angle = mod(PMOM_o(y1:y2, x1:x2)-kps(k,end), pi); % 局部主方向图，取值范围：[0,pi)
    else
        angle = PMOM_o(y1:y2, x1:x2); % 局部主方向图，取值范围：[0,pi)
    end
    angle_bin = zeros(patch_size+1,patch_size+1)-1; % -1+1=0 表示不统计
    angle_bin(W+y1-y+1:W+y2-y+1, W+x1-x+1:W+x2-x+1) = floor(angle*NO/pi)+1; % 取值范围：[1,NBO]
